...
def main():
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            lq = train_data['lq']
            gt = train_data['gt']
            semantic = train_data['semantic']
            gt_edge = train_data['gt_edge']
            ...
            if mini_gt_size < gt_size:
                ...
                y1 = y0 + mini_gt_size
                lq = lq[:,:,x0:x1,y0:y1]
                semantic = semantic[:,:,x0:x1,y0:y1]

                gt = gt[:,:,x0*scale:x1*scale,y0*scale:y1*scale]
                gt_edge = gt_edge[:,:,x0*scale:x1*scale,y0*scale:y1*scale]
            ###-------------------------------------------

            
            model.feed_train_data({'lq': lq, 'gt':gt, 'semantic':semantic, 'gt_edge':gt_edge})
