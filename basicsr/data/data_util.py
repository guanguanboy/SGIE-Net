...
def paired_paths_from_folder_lolv2(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
...
def tripled_paths_from_folder(folders, keys, filename_tmpl):
    ...
    for idx in range(len(gt_paths)):
        ...
        sem_name = f'{filename_tmpl.format(basename)}'+'_semantic' + f'{ext_input}'
...
def tripled_paths_w_mask_from_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    ...
    for idx in range(len(gt_paths)):
        ...
        sem_path = sem_paths[idx]
        basename_input, ext_input = osp.splitext(osp.basename(sem_path))
        sem_name = f'{filename_tmpl.format(basename)}{ext_input}'
        sem_path = osp.join(sem_folder, sem_name)
        assert sem_name in sem_paths, (f'{sem_name} is not in '
                                           f'{sem_key}_paths.')
