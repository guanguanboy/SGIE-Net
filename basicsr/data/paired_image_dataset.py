...
from basicsr.data.data_util import (paired_paths_from_folder,
                                    ...
                                    paired_paths_from_folder_lolv2)
...
class Dataset_PairedWithGrayIllumImage_LOLv2(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(Dataset_PairedWithGrayIllumImage_LOLv2, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            self.paths = paired_paths_from_folder_lolv2(
                [self.lq_folder, self.gt_folder], ['lq', 'gt'],
                self.filename_tmpl)
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            'lq_path': lq_path,
            'gt_path': gt_path,
            'semantic': img_gray_illum
        }

    def __len__(self):
        return len(self.paths)
    
class Dataset_PairedWithGrayIllumImage(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(Dataset_PairedWithGrayIllumImage, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
...
class Dataset_PairedImage_LOLv2(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(Dataset_PairedImage_LOLv2, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
