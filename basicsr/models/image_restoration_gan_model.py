...
class ImageSamGANCleanModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageSamGANCleanModel, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        self.net_d = define_network(self.opt['network_d'])
        ...
        if train_opt.get('perceptual_opt'):
            cri_perceptual_type = train_opt['perceptual_opt'].pop('type')
            cri_perceptual_cls = getattr(loss_module, cri_perceptual_type)
        ...
        if train_opt.get('gan_opt'):
            cri_gan_type = train_opt['gan_opt'].pop('type')
            cri_gan_cls = getattr(loss_module, cri_gan_type)
    ...
    def setup_optimizers(self):
        ...
        if optim_type == 'Adam':
            self.optimizer_d = torch.optim.Adam(optim_params, **train_opt['optim_d'])
        elif optim_type == 'AdamW':
            self.optimizer_d = torch.optim.AdamW(optim_params, **train_opt['optim_d'])
        else:
            raise NotImplementedError(
                f'optimizer {optim_type} is not supperted yet.')
