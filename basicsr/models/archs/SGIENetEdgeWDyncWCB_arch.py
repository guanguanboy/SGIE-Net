...
class Illumination_seg_gray_ill_Estimator(nn.Module):
    def __init__(
        ...
        #self.conv2 = nn.Conv2d(n_fea_middle, n_fea_out, kernel_size=1, bias=True)
        self.calibration_pwconv =  nn.Conv2d(1, n_fea_middle, kernel_size=1, bias=True)
    ...
    def forward(self, rgb, edge):
        ...
        calibrated_feats = self.calibration_pwconv(edge)
...
class SGIENetEdgeWDyncWCB(nn.Module):
...
if __name__ == '__main__':
    ...
    net = SGIENetEdgeWDyncWCB(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
