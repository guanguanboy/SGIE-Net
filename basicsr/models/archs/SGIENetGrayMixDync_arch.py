...
class FeedForward(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias):
        ...
        #self.project_in = DRConv2d(dim, hidden_features*2, kernel_size=1)
        self.dwconv = nn.Conv2d(hidden_features*2, hidden_features*2, kernel_size=3, stride=1, padding=1, groups=hidden_features*2, bias=bias)

        self.project_out = nn.Conv2d(hidden_features, dim, kernel_size=1, bias=bias)
        #self.project_out = DRConv2d(hidden_features, dim, kernel_size=1)
    def forward(self, x):
        x = self.project_in(x)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias, isdynamic=False):
        super(Attention, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))

        self.qkv = nn.Conv2d(dim, dim*3, kernel_size=1, bias=bias)
        #self.qkv = DRConv2d(dim, dim*3, kernel_size=1)

        if isdynamic:
            self.qkv_dwconv = DRConv2d(dim*3, dim*3, kernel_size=1)
        else:
            self.qkv_dwconv = nn.Conv2d(dim*3, dim*3, kernel_size=3, stride=1, padding=1, groups=dim*3, bias=bias)
        #self.project_out = DRConv2d(dim, dim, kernel_size=1)
...
class EnhanceNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        self.middle_blks = SGTB(dim=chan, num_blocks=middle_blk_num, isdynamic=True)
...
class SGIENetGrayMixDync(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        self.estimator = Illumination_Estimator(width)
        #self.estimator = Illumination_seg_Estimator(width)
...
if __name__ == '__main__':
    ...
    net = SGIENetGrayMixDync(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
                      enc_blk_nums=enc_blks, dec_blk_nums=dec_blks).cuda()


    inp_shape = (3, 256, 256)

    inp_img = torch.randn(2, 4, 384, 384).cuda()
    output = net(inp_img)
    print(output.shape)
