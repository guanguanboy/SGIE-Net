...
class Illumination_seg_gray_ill_Estimator(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(n_fea_in, n_fea_middle * 2, kernel_size=1, bias=True)

        self.depth_conv = nn.Conv2d(
            n_fea_middle, n_fea_middle, kernel_size=5, padding=2, bias=True, groups=n_fea_in)
        
        self.dr_conv = DRConv2d(n_fea_middle*2, n_fea_middle*2, kernel_size=1)
...
class SGIENetEdgeWDyncWFeatsGatedGray(nn.Module):
    ...
    def forward(self, inp):
        ...
        calibrated_edge_map, seg_feats = self.estimator(img_rgb, seg_map)
...
if __name__ == '__main__':
    ...
    net = SGIENetEdgeWDyncWFeatsGatedGray(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
