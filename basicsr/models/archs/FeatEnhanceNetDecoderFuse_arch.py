...
class NAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        for num in enc_blk_nums:
            ...
            self.weight_list.append(torch.nn.Parameter(torch.tensor(0.01),requires_grad=True))

            self.downs.append(
                nn.Conv2d(chan, 2*chan, 2, 2)
            )

            self.fuse_transform.append(nn.Conv2d(width, chan, scale_factor, scale_factor))
        ...
        vit_dim = 1280
        transformer_dim = 256
        self.compress_vit_feat_list = []

        for i in range(len(enc_blk_nums)):
            self.compress_vit_feat_list.append(nn.Sequential(
        ...
        self.embedding_deep_encoder = nn.Sequential(
                                        nn.Conv2d(transformer_dim, transformer_dim*2, kernel_size=2, stride=2),
                                        LayerNorm2d(transformer_dim*2),
                                        nn.GELU(),
                                        nn.Conv2d(transformer_dim*2, transformer_dim*2, kernel_size=2, stride=2),
                                    )       
        self.adjust_layer = depthwise_block()
    ...
    def forward(self, inp, inter_feats, deep_feats):
        ...
        #转化intermediate features
        inter_feats_compress_list = []
        for i in range(len(self.compress_vit_feat_list)):
            inter_feats_compressed = self.compress_vit_feat_list[i].to(inter_feats[i].device)(inter_feats[i].permute(0, 3, 1, 2))
            inter_feats_compress_list.append(inter_feats_compressed)

        encs = []
        original_width = x.shape[2]
        original_height = x.shape[3]
        for encoder, down, inter_feat, fuse_transform in zip(self.encoders, self.downs, inter_feats_compress_list, self.fuse_transform):

            x = encoder(x)
            inter_feat = F.interpolate(inter_feat, [original_width, original_height])
            inter_feat_transformed = fuse_transform(inter_feat)

            encs.append(x + inter_feat_transformed)
            x = down(x)

        x = self.middle_blks(x) #torch.Size([2, 512, 16, 16])
...
class FeatEnhanceNetDecoderFuse(nn.Module):
...
if __name__ == '__main__':
    ...
    samenhancenet = FeatEnhanceNetDecoderFuse(img_channel=3, width=width, middle_blk_num=middle_blk_num, enc_blk_nums=enc_blks, dec_blk_nums=dec_blks).cuda()
    
    input_sam_image = torch.randn(2, 3, 384, 384).cuda()
