...
class EnlightenTransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(EnlightenTransformer, self).__init__()
...
class SSG_V0(nn.Module):
    ...
    def __init__(
        ...
        enlighten_transformer: EnlightenTransformer,
        image_decoder: ImageDecoderCNN,
    ) -> None:
        """
...
def build_ssg():
    ...
    ssg = SSG_V0(
        ...
        enlighten_transformer= EnlightenTransformer(upscale=1, in_chans=4, img_size=(image_width, image_height),
                   window_size=window_size, depths=[6, 6, 6, 6],
                   embed_dim=embed_dim, num_heads=[6, 6, 6, 6], mlp_ratio=2,          
                   upsampler='pixelshuffle',resi_connection='1conv'),
        image_decoder= ImageDecoderCNN(nf=embed_dim, back_RBs=1)
    )
...
def test_hat():
    ...
    width = 256
    window_size = 16
    model = EnlightenTransformer(upscale=1, in_chans=4, img_size=(width, height),
                   window_size=window_size, depths=[6, 6, 6, 6],
                   embed_dim=120, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='pixelshuffle',resi_connection='1conv')
    print(model)
    #print(height, width, model.flops() / 1e9)

    #x = torch.randn((1, 4, width, height))
    #x = model(x)
    input_x_feat = torch.randn((1, 120, 64, 64))
    output_x_feat = model(input_x_feat)
    print(output_x_feat.shape)
...
if __name__ == '__main__':
    ...
    print('light transformer output shape=', y.shape) #1 3 256 256

    image_encoder = ImageEncoderCNN(nf=120, front_RBs=1)
    image_embedding,L1_fea_1,L1_fea_2,L1_fea_3 = image_encoder(x)
    print(image_embedding.shape) #1 120 64 64

    height = 256 
    width = 256
    window_size = 16
    enlighten_transformer = EnlightenTransformer(upscale=1, in_chans=4, img_size=(width, height),
                   window_size=window_size, depths=[6, 6, 6, 6],
                   embed_dim=120, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='pixelshuffle',resi_connection='1conv')
    enlighten_embedding = enlighten_transformer(image_embedding)

    print("enlighten_embedding.shape=", enlighten_embedding.shape)
