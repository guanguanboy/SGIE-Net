...
class Illumination_seg_gray_ill_Estimator(nn.Module):
    def __init__(
        ...
        #self.conv2 = nn.Conv2d(n_fea_middle, n_fea_out, kernel_size=1, bias=True)
        self.calibration = nn.Conv2d(2,1,kernel_size=3, padding=1, bias=True)
    ...
    def forward(self, rgb, edge):
        ...
        calibrated_edge_map = self.calibration(edge_illu)
        return calibrated_edge_map, illu_fea
            
class SGIENetEdgeWDyncWGray(nn.Module):
...
if __name__ == '__main__':
    ...
    net = SGIENetEdgeWDyncWGray(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
