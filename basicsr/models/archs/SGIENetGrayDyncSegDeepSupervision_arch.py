...
class EnhanceNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        self.output_coverters = nn.ModuleList()
        ...
        for num in dec_blk_nums:
            ...
            self.output_coverters.append(
                nn.Conv2d(in_channels=chan, out_channels=3, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
            )
    ...
    def forward(self, inp, seg_feats):
        ...
        for decoder, up, enc_skip, output_converter in zip(self.decoders, self.ups, encs[::-1], self.output_coverters):
            x = up(x)
            x = x + enc_skip
            seg_feats = seg_feats_store_list[self.level_count-1-level_curr]
            x = decoder([x, seg_feats])
            
            output = output_converter(x)
...
class Illumination_seg_gray_ill_Estimator(nn.Module):
    def __init__(
            self, n_fea_middle, n_fea_in=2, n_fea_out=3):  #__init__部分是内部属性，而forward的输入才是外部输入
    ...
    def forward(self, seg_map, gray_illumin):
        # img:        b,c=3,h,w
        # mean_c:     b,c=1,h,w
        ...
        #mean_c = img.mean(dim=1).unsqueeze(1)
        # stx()
        input = torch.cat([seg_map,gray_illumin], dim=1)
...
class SGIENetGrayDyncSegDeepSupervision(nn.Module):
    ...
    def forward(self, inp):
        ...
        seg_feats, illu_map = self.estimator(seg_map, gray_ill)
...
if __name__ == '__main__':
    ...
    net = SGIENetGrayDyncSegDeepSupervision(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
                      enc_blk_nums=enc_blks, dec_blk_nums=dec_blks).cuda()


    inp_shape = (3, 256, 256)

    inp_img = torch.randn(1, 5, 256, 256).cuda()
    output_list = net(inp_img)
