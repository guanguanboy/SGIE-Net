...
class GFFM(nn.Module):
    def __init__(self, in_channels, bias=True, padding_mode='reflect', **kwargs) -> None:
        super().__init__()
...
class EnhanceNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        self.encoder_gffm = nn.ModuleList()
        self.decoder_gffm = nn.ModuleList()
        ...
        for num in enc_blk_nums:
            ...
            self.encoder_gffm.append(
                GFFM(chan)
            )
            chan = chan * 2

        self.middle_gffm = GFFM(chan)
    ...
    def forward(self, inp, seg_feats):
        ...
        for encoder, down, seg_feats_down, e_gffm in zip(self.encoders, self.downs, self.seg_feats_downs, self.encoder_gffm):
            seg_feats = e_gffm(seg_feats)
        ...
        seg_feats = self.middle_gffm(seg_feats)
        #这里也需要过滤，过滤之后才能加进去
        x = self.middle_blks([x,seg_feats])

        level_curr = 0
        for decoder, up, enc_skip, d_gffm in zip(self.decoders, self.ups, encs[::-1], self.decoder_gffm):
            x = up(x)
            x = x + enc_skip
            seg_feats = seg_feats_store_list[self.level_count-1-level_curr]
            #这里对seg_feats进行过滤
            seg_feats = d_gffm(seg_feats)
...
class SegmentationPriorGenerator(nn.Module):
    def __init__(
            self, n_fea_middle, n_fea_in=4, n_fea_out=3):  #__init__部分是内部属性，而forward的输入才是外部输入
        super(SegmentationPriorGenerator, self).__init__()
        ...
        #self.conv2 = nn.Conv2d(n_fea_middle, n_fea_out, kernel_size=1, bias=True)
        #self.pwconv = nn.Conv2d(n_fea_middle, n_fea_middle * 2, 1, 1, 0, bias=True)

        #self.pwconv_project = nn.Conv2d(n_fea_middle, n_fea_middle, kernel_size=1, bias=True)

        self.calibration_pwconv =  nn.Conv2d(2, n_fea_middle, kernel_size=1, bias=True)
        self.calibration_dwconv = nn.Conv2d(
            n_fea_middle, n_fea_middle, kernel_size=3, padding=1, bias=True, groups=n_fea_middle)
        self.calibration_out = nn.Conv2d(n_fea_middle, 1, kernel_size=1, bias=True)
    def forward(self, rgb, edge):
        ...
        x_1 = self.conv1(input)
        #illu_fea = self.depth_conv(x_1)
        dy_illu_fea = self.dr_conv(x_1)
        #illu_map = self.conv2(illu_fea)
        illu_fea = self.depth_conv(dy_illu_fea)
        #x1, x2 = dy_illu_fea.chunk(2, dim=1)
        #x = F.gelu(x1) * x2
        #illu_fea = self.pwconv_project(x)

        edge_illu = torch.cat([edge, mean_c], dim=1)

        calibrated_feats = self.calibration_pwconv(edge_illu)
        calibrated_feats = self.calibration_dwconv(calibrated_feats)
        calibrated_edge_map = self.calibration_out(calibrated_feats)
        return calibrated_edge_map, illu_fea
            
class SGIENetEdgeWDyncWFeatsGatedGraySimple(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        self.prior_generator = SegmentationPriorGenerator(width)
    ...
    def forward(self, inp):
        ...
        calibrated_edge_map, seg_feats = self.prior_generator(img_rgb, seg_map)
        ...
        input = torch.cat([img_rgb, calibrated_edge_map],dim=1)
        x = self.enhancer(input, seg_feats)
...
if __name__ == '__main__':
    ...
    net = SGIENetEdgeWDyncWFeatsGatedGraySimple(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
                      enc_blk_nums=enc_blks, dec_blk_nums=dec_blks).to(device)


    inp_shape = (3, 256, 256)

    inp_img = torch.randn(1, 4, 256, 256).to(device)
    output = net(inp_img)
    print(output.shape)
