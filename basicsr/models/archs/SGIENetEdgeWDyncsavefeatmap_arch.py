...
class SGIENetEdgeWDyncsavefeatmap(nn.Module):
    ...
    def forward(self, inp):
        ...
        feats_file_name = f'results/featmaps/{featmap_index}_feats_a.png'

        save_feature_map(seg_feats[:,0:10,:,:], feats_file_name)

        feats_file_name = f'results/featmaps/{featmap_index}_feats_b.png'

        save_feature_map(seg_feats[:,10:20,:,:], feats_file_name)

        feats_file_name = f'results/featmaps/{featmap_index}_feats_c.png'

        save_feature_map(seg_feats[:,20:,:,:], feats_file_name)
