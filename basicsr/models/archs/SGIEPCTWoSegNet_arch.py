...
class RestormerTransformerBlock(nn.Module):
    def __init__(self, dim, num_blocks, num_heads=4, ffn_expansion_factor= 2.66,
        ...
        super(RestormerTransformerBlock, self).__init__()
        self.blocks = nn.ModuleList([])
        for _ in range(num_blocks):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        for num in enc_blk_nums:
            self.encoders.append(
                    ...
                    RestormerTransformerBlock(chan,num)
        ...
        self.middle_blks = RestormerTransformerBlock(dim=chan, num_blocks=middle_blk_num)
...
class SGIEPCTWoSegNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        #self.encoder = EnhanceNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num, enc_blk_nums=enc_blk_nums, dec_blk_nums=dec_blk_nums)
            
        self.decoder = lambda intermediates, img: (intermediates)

        self.get_params = nn.Conv2d(input_dim, self.out_dim, kernel_size=1)  
        
        self.depth_conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=1, bias=True)
        self.feature_converter = nn.Conv2d(in_channels=width, out_channels=width, kernel_size=3, padding=1, stride=1, groups=width, bias=True)
        self.depth_conv2 = nn.Conv2d(in_channels=width, out_channels=width, kernel_size=1, bias=True)
        self.segfeats_construter = nn.Conv2d(in_channels=width, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1, bias=True)
        
        self.ending_depth_conv = nn.Conv2d(in_channels=width, out_channels=width, kernel_size=1, bias=True)

        self.ending = nn.Conv2d(in_channels=width, out_channels=12, kernel_size=3, padding=1, stride=1, groups=1, bias=True)

        self.enhancer = EnhanceNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num, enc_blk_nums=enc_blk_nums, dec_blk_nums=dec_blk_nums)
    ...
    def forward(self, inp):
        ...
        #获取pixel transform fuction的结果
        # Low resolution branch
        #x = img_rgb
        
        #intermediates = self.encoder(x) # intermediates ([1, 12, 256, 256])

        x = self.depth_conv1(img_rgb)
        seg_feats = self.feature_converter(x)

        seg_feats = self.depth_conv2(seg_feats)
        seg_feats = self.segfeats_construter(seg_feats)

        seg_feats = self.ending_depth_conv(seg_feats)
        intermediates = self.ending(seg_feats)
        ...
        output_pixel_trans = self.PCT(img_rgb, params) #(1, 12, 256, 256)

        x = self.enhancer(output_pixel_trans, seg_feats)

        return [x, output_pixel_trans]
...
if __name__ == '__main__':
    ...
    net = SGIEPCTWoSegNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
                      enc_blk_nums=enc_blks, dec_blk_nums=dec_blks).cuda()


    inp_shape = (3, 256, 256)

    inp_img = torch.randn(2, 4, 256, 256).cuda()
    output = net(inp_img)
