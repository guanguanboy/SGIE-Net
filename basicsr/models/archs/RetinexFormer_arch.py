...
class Illumination_Estimator(nn.Module):
    def __init__(
            self, n_fea_middle, n_fea_in=5, n_fea_out=3):  #__init__部分是内部属性，而forward的输入才是外部输入
    ...
    def forward(self, img, sematic_mask):
        # img:        b,c=3,h,w
        # mean_c:     b,c=1,h,w
        ...
        # stx()
        input = torch.cat([img,mean_c,sematic_mask], dim=1)
...
class RetinexFormer_Single_Stage(nn.Module):
    ...
    def forward(self, img):
        ...
        illu_fea, illu_map = self.estimator(img_rgb, sematic_mask)
