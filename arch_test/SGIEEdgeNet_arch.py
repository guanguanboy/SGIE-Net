...
##########################################################################
## Multi-DConv Head Transposed Self-Attention (MDTA)
class Attention_origin(nn.Module):
    def __init__(self, dim, num_heads, bias):
        super(Attention_origin, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads=4, ffn_expansion_factor= 2.66,
        ...
        self.attn = Attention_origin(dim, num_heads, bias)
...
class PriorConstructionNet(nn.Module):
...
class SGIEEdgeNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        #self.depth_conv2 = nn.Conv2d(in_channels=width, out_channels=1, kernel_size=1, bias=True)

        #self.edge_construter = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=3, padding=1, stride=1, groups=1, bias=True)
        
        self.edge_construter = PriorConstructionNet(img_channel=4, width=width, middle_blk_num=1, enc_blk_nums=[1,1,1], dec_blk_nums=[1,1,1])
    ...
    def forward(self, inp):
        ...
        #edge_map = self.depth_conv2(seg_feats)
        #edge_map = self.edge_construter(edge_map)

        edge_map, seg_feats = self.edge_construter(inp)

        x = self.enhancer(img_rgb, seg_feats)
...
if __name__ == '__main__':
    ...
    net = SGIEEdgeNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
